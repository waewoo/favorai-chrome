...
export async function queryMistral(url, key, model, prompt, systemPrompt, signal, debugMode, maxTokens = 131072) {
  const endpoint = `${url.replace(/\/$/, '')}/chat/completions`;

  if (debugMode) {
    console.log('=== DEBUG: Mistral Request ===');
  ...
  const response = await retryTransientRequest(async () => {
    ...
    if (!currentResponse.ok) {
      ...
      const e = new Error(formatErrorMessage('Mistral', currentResponse.status, err));
